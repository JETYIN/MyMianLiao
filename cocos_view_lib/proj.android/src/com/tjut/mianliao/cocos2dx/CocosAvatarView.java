...
public class CocosAvatarView extends Cocos2dxGLSurfaceView implements Cocos2dxHelperListener {
    ...
    private static OnNightAnimFinishListener sOnNightAnimFinishListener;
    ...
    @Override
    public void showEditTextDialog(String pTitle, String pMessage, int pInputMode, int pInputFlag, int pReturnType,
            int pMaxLength) {
    }
    ...
    public static void setOnNightAnimFinishListener(OnNightAnimFinishListener listener) {
        sOnNightAnimFinishListener = listener;
    }
    ...
    public static void changeSuit(final int side, final HashMap<String, String> suit,final String atlasPath) {
        if (sAvatarLoaded && suit != null) {
            Cocos2dxHelper.runOnGLThread(new Runnable() {
                @Override
                public void run() {
                    for (Entry<String, String> entry : suit.entrySet()) {
                        nativeChangeSuit(side, entry.getKey(), entry.getValue(),atlasPath);
                    }
                }
            });
        }
    ...
    public static void changeSuit(final int side, final String originalLayer, final String replaceImageName,final String atlasPath) {
        if (sAvatarLoaded) {
            Cocos2dxHelper.runOnGLThread(new Runnable() {
                @Override
                public void run() {
                    nativeChangeSuit(side, originalLayer, replaceImageName,atlasPath);
                }
            });
        }
    }
    ...
    public static void loadClothesData(final String suitName, final boolean isSandBoxVersionExists) {
        Cocos2dxHelper.runOnGLThread(new Runnable() {
            @Override
            public void run() {
    ...
    public static void ShowDayNightAnim() {
        if (sAvatarLoaded) {
            Cocos2dxHelper.runOnGLThread(new Runnable() {
                @Override
                public void run() {
                    nativeShowDayNightAnim();
                }
            });
        }
    ...
    private static void onNightAnimFinish()
    {

        if(sOnNightAnimFinishListener!=null)
        {
            sOnNightAnimFinishListener.onNightAnimFinish();
        }
    }
    ...
    private static native void nativeChangeSuit(int side, String originalLayer, String replaceImageName,String atlasPath);
    ...
    private static native void nativeShowDayNightAnim();

    public interface OnAvatarLoadedListener {
        public void onAvatarLoaded();
    }

    public interface OnNightAnimFinishListener {
