...
public class TaskActivity extends BaseActivity {
	...
	private ThemeTextView mShowKernel;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mShowKernel = (ThemeTextView) findViewById(R.id.tv_show_my_kernel);
	...
	private void checkDayNightUI() {
		if (mIsNightMode) {
			findViewById(R.id.sv_task).setBackgroundResource(R.drawable.bg);
		}
	}
	...
	private class MyTaskTask extends MsTask {
		...
		protected void onPostExecute(MsResponse response) {
			if (response.isSuccessful()) {
				...
				getTaskProgress();
				mTvDayTitle.setText("每日任务("+mDayTaskProgress+"/"+(mTask.getDailyInfo()).getInfo().size()+")");
				mTvNewTitle.setText("新手任务("+mNewTaskProgress+"/"+(mTask.getRookieInfo()).getInfo().size()+")");
				mTvCollegeTitle.setText("漫游高校解锁任务("+myCollegeProgress+"/"+5+")");
			}
		}
	}
	...
	OnClickListener mListen = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (mFlagTask.getProcess() < mFlagTask.getMax()) {
				if (mSkipFlag.equals(getString(R.string.task_day_sign))) {
				...
				} else if (mSkipFlag.equals(getString(R.string.task_school_post))) {
					...
					startActivity(intent);
				} else if (mSkipFlag.equals(getString(R.string.task_tribe_post))) {
					intent.setClass(TaskActivity.this, MainActivity.class);
					...
					intent.putExtra(MainActivity.EXTRA_POST_TAB_INDEX, 1);
					startActivity(intent);
				} else if (mSkipFlag.equals(getString(R.string.note_post_note))) {
					intent.setClass(TaskActivity.this,
							TakeNoticesActivity.class);
					startActivity(intent);
				} else if (mSkipFlag.equals(getString(R.string.task_add_friend))) {
					if (mIsNightMode) {
						toast("夜晚就要狂嗨，好友神马的都是浮云");
					} else {
						intent.setClass(TaskActivity.this,
								AddContactActivity.class);
						startActivity(intent);
					}
				} else if (mSkipFlag.equals(getString(R.string.iv_title))) {
					intent.setClass(TaskActivity.this, IdVerifyActivity.class);
					startActivity(intent);
				} else if (mSkipFlag.equals(getString(R.string.task_interactive_cartoon))) {
					intent.setClass(TaskActivity.this, MainActivity.class);
		...
		if (mTask.getCollegeInfo().getInfo().size() == 0) {
		    ...
		    for (int i = 0; i < (mTask.getCollegeInfo().getInfo().size()); i++) {
		        if (mTask.getCollegeInfo().getLevel().size() < 5) {
		            myCollegeProgress = mTask.getCollegeInfo().getLevel().size() - 1;
		        } else {
		            int num = 0;
		            for (int j = 0; j < (mTask.getCollegeInfo().getInfo().size()); j++) {
