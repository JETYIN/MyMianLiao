...
public class ChatAdapter extends BaseAdapter implements OnCompletionListener,
    ...
    private boolean isGroupChat, mShowMyName, mIsNightMode;
    ...
    private void updateUserView(View view, ChatRecord record, boolean isMe) {
        ...
        if (user != null) {
            view.findViewById(R.id.iv_vip_bg).setVisibility(user.vip && !mIsNightMode ?
    ...
    private void viewPicture(ChatRecord record) {
        if (record != null) {
            String url = record.isFileExist() ? record.filePath : record.url;
            Utils.viewImages(mContext, mImageUrls, mImageUrls.indexOf(url));
        }
    }
