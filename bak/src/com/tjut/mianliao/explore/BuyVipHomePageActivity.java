...
public class BuyVipHomePageActivity extends BaseActivity implements OnClickListener,
	...
	private void checkDayNightUI() {
		if (mIsNightMode) {
            findViewById(R.id.ll_vip_home).setBackgroundResource(R.drawable.bg);
            mLvVipInfo.setBackgroundColor(0X32000000);
        }
	}

    private void updateTitle() {
        if (mUserInfo != null) {
            getTitleBar().setTitle(mUserInfo.vip ? "续费面聊会员" : "购买面聊会员");
        } else {
            getTitleBar().setTitle("购买面聊会员");
            getUserInfo();
        }
    }
    ...
    private void showDiscardDialog() {
        if(mDisDialog == null) {
            mDisDialog = new LightDialog(this).setTitleLd("确定要放弃付款?")
