...
public class AvatarMarketActivity extends BrowserActivity implements
		OnClickListener, DialogInterface.OnClickListener, IMResourceListener,
		OnAvatarLoadedListener {
	...
	private boolean processUrl(String url) {
		...
		if (url.startsWith(MsRequest.IMRW_CLICK.getToLocalUrl())) {
			...
			if (mResType == IMResource.TYPE_BACKGROUND
					|| mResType == IMResource.TYPE_COURSE_BACKGROUD
		...
		if (mResVip) {
			if (mUserInfo.vip) {
				mPayDialog.setMessage(getString(R.string.pay_resource_vip_msg,
						getVipPriceStr(), getResTypeName()));
				mBuyVip = false;
			} else {
				mPayDialog.setMessage("该" + getResTypeName()
						+ "为VIP专享，立刻升级成为VIP?");
				mBuyVip = true;
			}
			...
			if (mUserInfo.vip) {
				...
				mPayDialog.setMessage(getString(
						R.string.pay_resource_novip_msg, getPriceStr(),
						getResTypeName()));
			}
		}
		mPayDialog.show();
