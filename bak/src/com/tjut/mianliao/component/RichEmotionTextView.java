...
//                            Toast.makeText(mContext, "atUser == null", Toast.LENGTH_SHORT).show();
    ...
    private boolean hasDetail(CfPost post) {
        if (post.threadType == CfPost.THREAD_TYPE_NORMAL || post.threadType == CfPost.THREAD_TYPE_TXT_VOTE
                || post.threadType == CfPost.THREAD_TYPE_PIC_VOTE ||  post.threadType == CfPost.THREAD_TYPE_RICH_MEDIA) {
            return true;
        }
        return false;
    }
