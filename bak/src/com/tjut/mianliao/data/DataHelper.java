...
public class DataHelper {
    ...
    public static ArrayList<UserEntry> loadUserEntries(Context context) {
        ...
        int colSubType = c.getColumnIndex(UserEntry.SUB_TYPE);
        int colSubStatus = c.getColumnIndex(UserEntry.SUB_STATUS);

        while (c.moveToNext()) {
            UserEntry user = new UserEntry(c.getString(colJid), c.getString(colSubType), c.getString(colSubStatus));
            users.add(user);
        }
        c.close();
        db.close();
    ...
    private static ContentValues fillUserEntry(UserEntry user) {
        ...
        cv.put(UserEntry.SUB_TYPE, user.subType.toString());
        cv.put(UserEntry.SUB_STATUS, user.subStatus == null ? null : user.subStatus.toString());
        Utils.logD(
                TAG,
                "fillUserEntry:" + user.jid + ":" + user.subType.toString() + ":"
                        + cv.getAsString(UserEntry.SUB_STATUS));
        return cv;
    }
    ...
    private static void loadChatRecords(Cursor c, ArrayList<ChatRecord> chatRecords) {
        ...
        while (c.moveToNext()) {
            ...
            record.voiceLength = c.getFloat(colVoiceLength);
    ...
    public static synchronized void deletePostInfo(Context context, int forumId, boolean isNightPost) {
        if (forumId <= 0) {
            return;
        }
        SQLiteDatabase db = getWritableDatabase(context);
        ...
        int isNight = isNightPost ? 1 : 0;
        db.delete(CachePostInfo.TABLE_NAME, CachePostInfo.FORUM_ID + "='" +forumId + "' and " +
                CachePostInfo.IS_NIGHT_POST + "='" + isNight + "'", null);
        db.close();
        Utils.logD(TAG, "Delete post info succ, forumId = " + forumId);
    }
    
    public static synchronized void deletePostInfoBySchoolId(Context context, int schoolId, boolean isNightPost) {
        if (schoolId <= 0) {
            return;
        }
        SQLiteDatabase db = getWritableDatabase(context);
    ...
    public static ArrayList<CfPost> loadPostsInfo(Context context, boolean isNightPost, long forumId) {
        ArrayList<CfPost> posts = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase(context);
        int isNight = isNightPost ? 1 : 0;
        Cursor c = db.query(CachePostInfo.TABLE_NAME, null, CachePostInfo.IS_NIGHT_POST +"='" + isNight + "' and " 
                + CachePostInfo.FORUM_ID + "='" + forumId + "'", null, null, null, null);
        loadCachePosts(context, c, posts, isNightPost);
        c.close();
        db.close();
        Utils.logD(TAG, "load posts info with total " + posts.size() +",and the isNightPost : " + isNightPost);
        return posts;
    }
    
    public static ArrayList<CfPost> loadPostsInfoBySchoolId(Context context, boolean isNightPost, long schoolId) {
        ArrayList<CfPost> posts = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase(context);
        int isNight = isNightPost ? 1 : 0;
        Cursor c = db.query(CachePostInfo.TABLE_NAME, null, CachePostInfo.IS_NIGHT_POST +"='" + isNight + "' and " 
    ...
    private static void loadCachePosts(Context context, Cursor c, ArrayList<CfPost> posts, boolean isNight) {
        ...
        while(c.moveToNext()){
            ...
            post.userInfo = loadCacheUserInfo(context, c.getLong(columnUserId), isNight);
            post.likedUsers = getLikedUser(context, c.getString(columnLikedUserIds), isNight);
            posts.add(post);
        }
    }
    
    private static ArrayList<UserInfo> getLikedUser(Context context, String ids, boolean isNight) {
        ...
        for (int i = 0; i < userIds.length; i++) {
            UserInfo userInfo = loadCacheUserInfo(context, Integer.parseInt(userIds[i]), isNight);
            if (userInfo != null) {
                users.add(userInfo);
            }
        }
    ...
    public static synchronized void insertCacheUserInfo(Context context, UserInfo userInfo, boolean isNight) {
        if (userInfo == null) {
            return;
        }
        SQLiteDatabase db = getWritableDatabase(context);
        ...
        long id = db.insert(CacheUserInfo.TABLE_NAME, null, fillCacheUserInfo(userInfo, isNight));
        db.close();
        if (id == -1) {
    ...
    public static UserInfo loadCacheUserInfo(Context context, long uid, boolean isNight) {
        if (uid <= 0) {
            return null;
        }
        SQLiteDatabase db = getReadableDatabase(context);
        ...
        Cursor c = db.query(CacheUserInfo.TABLE_NAME, null, CacheUserInfo.USER_ID + "='" + uid + "'",
                null, null, null, null);
        ArrayList<UserInfo> users = new ArrayList<>();
        loadCacheUserInfo(c, users, isNight);
        c.close();
        db.close();
        return users == null || users.size() == 0 ? null : users.get(0);
    }
    
    public static synchronized void updateCacheUserInfo(Context context, UserInfo user, boolean isNight) {
        if (user == null) {
            return ;
        }
        SQLiteDatabase db = getWritableDatabase(context);
        ...
        int index = db.update(CacheUserInfo.TABLE_NAME, fillCacheUserInfo(user, isNight),
    ...
    private static void loadCacheUserInfo(Cursor c, ArrayList<UserInfo> users, boolean isNight) {
        ...
        while (c.moveToNext()) {
            ...
            if (isNight) {
                user.nickname = c.getString(columnNickNight);
            } else {
            ...
            if (isNight) {
                user.setAvatar(c.getString(columnAvatarNight));
            } else {
    ...
    private static ContentValues fillCacheUserInfo(UserInfo userInfo, boolean isNight) {
        ...
        if (!isNight) {
            ...
            value.put(CacheUserInfo.NICK_NIGHT, userInfo.nickname);
        }
        ...
        if (!isNight) {
            ...
            value.put(CacheUserInfo.AVATAR_NIGHT, userInfo.getAvatar());
        }
    ...
    public static synchronized void insertFastEntranceInfos(Context context, ArrayList<FastEntranceInfo> entrances) {
        ...
        db.beginTransaction();
        try {
            for (FastEntranceInfo info : entrances) {
                long id = db.insert(FastEntranceInfo.TABLE_NAME, null, fillEntranceInfo(info));
                if (id == -1) {
                    Utils.logW(TAG, "Failed inserting fast entrance data: " + info.name);
                }
            }
            db.setTransactionSuccessful();
        } finally {
    ...
    public static synchronized void deleteFastEntranceInfo(Context context) {
        SQLiteDatabase db = getWritableDatabase(context);
        if (db == null) {
            return;
        }
        db.delete(FastEntranceInfo.TABLE_NAME, null, null);
        db.close();
    }
    
    public static ArrayList<FastEntranceInfo> loadFastEntranceInfo(Context context) {
        ArrayList<FastEntranceInfo> infos = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase(context);
        if (db == null) {
            return infos;
        }
        Cursor cursor = db.query(FastEntranceInfo.TABLE_NAME, null, null, null, null, null, null);
        fillFastEntranceInfo(cursor, infos);
        cursor.close();
        db.close();
        return infos;
    }
    
    private static void fillFastEntranceInfo(Cursor cursor, ArrayList<FastEntranceInfo> infos) {
        int columnImgRes = cursor.getColumnIndex(FastEntranceInfo.IMG_RES);
        int columnName = cursor.getColumnIndex(FastEntranceInfo.NAME);
        int columnClassName = cursor.getColumnIndex(FastEntranceInfo.CLASS_NAME);
        while(cursor.moveToNext()){
            FastEntranceInfo info = new FastEntranceInfo();
            info.imgResBig = cursor.getInt(columnImgRes);
            info.name = cursor.getString(columnName);
            info.className = cursor.getString(columnClassName);
            infos.add(info);
        }
    }

    private static ContentValues fillEntranceInfo(FastEntranceInfo info) {
        ContentValues value = new ContentValues();
        value.put(FastEntranceInfo.IMG_RES, info.imgResBig);
        value.put(FastEntranceInfo.NAME, info.name);
        value.put(FastEntranceInfo.CLASS_NAME, info.className);
        return value;
    }
