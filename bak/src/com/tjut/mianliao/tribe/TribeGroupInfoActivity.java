...
public class TribeGroupInfoActivity extends BaseActivity implements OnClickListener, TribeChatListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mRoomInfo.roomAvatar != null && !mRoomInfo.roomAvatar.equals("")){
            ...
            placeholder(R.drawable.chat_botton_bg_faviconboy).into(mIvAvatar);
        }
    ...
    private void checkDayNightUI() {
        if (mIsNightMode) {
            findViewById(R.id.rl_content).setBackgroundResource(R.drawable.bg);
        }
    }
    ...
    private class GroupMenberAdapter extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            View view;
            if (convertView == null) {
                view = mInflater.inflate(R.layout.list_item_group_member, parent, false);
            } else {
                view = convertView; 
            }
            UserInfo user = getItem(position);
            AvatarView mUserAvatar = (AvatarView) view.findViewById(R.id.iv_member_avatar);
            Picasso.with(TribeGroupInfoActivity.this).load(user.getAvatar())
                .placeholder(R.drawable.chat_botton_bg_faviconboy).into(mUserAvatar);
            view.setOnClickListener(new OnClickListener() {
                
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent();
                    intent.setClass(TribeGroupInfoActivity.this, TribeChatRoomUsersActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.tv_go_chat:
                ...
                mTribeChatManager.accessChatRoom(mUserId, 
                        mRoomInfo.tribeId, String.valueOf(mRoomInfo.roomId));
                break;
            ...
            case R.id.rl_invite_friend:
                intent.setClass(TribeGroupInfoActivity.this, AddFriendToGroupActivity.class);
                intent.putExtra(GroupChatInfoActivity.ADD_FRIEND, true);
                startActivityForResult(intent, ADD_FRIEND_TO_TRIBE_CHAT);
                break;
            case R.id.rl_master:
                intent.setClass(TribeGroupInfoActivity.this, ProfileActivity.class);
    ...
    public void onSuccess(int type, Object obj) {
        switch (type) {
            case TribeChatManager.TYPE_ADD_ROOM:
                ...
                intent.putExtra(ChatActivity.EXTRA_CHAT_TARGET, createTribeChatTarget(mRoomInfo.roomId));
    ...
    private class getGroupUsersTask extends MsTask {
        ...
        public getGroupUsersTask() {
            super(TribeGroupInfoActivity.this,MsRequest.TRIBE_GET_CHAT_ROOM_USERS);
        }
    ...
    private String createTribeChatTarget(int roomId) {
        return roomId + "tribe@groupchat." + Utils.getChatServerDomain();
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == ADD_FRIEND_TO_TRIBE_CHAT && resultCode == RESULT_OK) {
            ArrayList<UserEntry> users = data.getParcelableArrayListExtra(
...
//            ArrayList<UserInfo> 
            ...
            mTobeAddUsers.clear();
        } else if (requestCode == FOR_CHAT && resultCode == ChatActivity.RESULT_DELETED) {
            finish();
        }
