...
import com.tjut.mianliao.mycollege.MyVisitorActivity;
import com.tjut.mianliao.mycollege.PraisemeUserActivity;
...
public class ProfileActivity extends BaseActivity implements OnClickListener, SubRequestListener, ImageResultListener,
    ...
    private void checkDayNightUI() {
        if (mIsNightMode) {
            findViewById(R.id.sv_profile).setBackgroundResource(R.drawable.bg);
        } else {
            findViewById(R.id.sv_profile).setBackgroundColor(0XFFF2F2F2);
        }
    }
    ...
    private void showBasicUserInfo() {
        ...
        findViewById(R.id.iv_vip_bg).setVisibility(mUserInfo.vip && !mIsNightMode ? View.VISIBLE : View.INVISIBLE);
        ...
        LinearLayout llWonupCount = (LinearLayout) findViewById(R.id.ll_won_up_count);
        mTvFlowerCount = (TextView) findViewById(R.id.tv_visitor_count);
        LinearLayout llVisitorpCount = (LinearLayout) findViewById(R.id.ll_visitor_count);
        ...
        llVisitorpCount.setOnClickListener(this);
        showScene();
        hideIconAndButton();
    }

    private void hideIconAndButton() {
        if (mIsNightMode) {
            findViewById(R.id.iv_menu).setVisibility(View.GONE);
            findViewById(R.id.btn_chat).setVisibility(View.GONE);
            findViewById(R.id.btn_add_contact).setVisibility(View.GONE);
        }
    }
    ...
    private void unsubscribe() {
        if (mConnectionManager.isXmppConnected()) {
            // getTitleBar().showProgress();
            mSubscriptionHelper.unsubscribe(mUserInfo.jid);
        } else {
            toast(R.string.disconnected);
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tv_name:
                if (mUserInfo.isMine(this) && !mIsNightMode) {
