...
public class MyMedalsActivity extends BaseActivity implements OnClickListener{
    ...
    private void checkDayNightUI() {
        if (mIsNightMode) {
            mSvMedal.setBackgroundResource(R.drawable.bg);
        } else {
            mSvMedal.setBackgroundColor(0XFFF2F2F2);
        }
    }
    ...
    private void updataView() {
        ...
        mTvShowing.setText("正在展示（"+ mShowMadels.size() + "/5)");
    }
    ...
    private View getView(Medal mMedal, int Type) {
        ...
        if (Type == 0) {
            ...
            if (mIsNightMode) {
                mMedalBt.setImageResource(R.drawable.repeat_black);
            } else {
